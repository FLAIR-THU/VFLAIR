...
class GPT2ForQuestionAnswering_pretrained(GPT2PreTrainedModel):
    def __init__(self, global_gpt, qa_outputs):
...
class GlobalGPT2Classifier(GPT2PreTrainedModel):
    def __init__(self, global_gpt, output_dim):
        super().__init__(global_gpt.config)
        self.num_labels = output_dim #global_gpt.config.num_labels
        self.transformer = global_gpt #GPT2Model(config)
        self.score = nn.Linear(global_gpt.config.n_embd, self.num_labels, bias=False)
...
class LocalGPT2Model(GPT2PreTrainedModel):
    def __init__(self, full_gpt, num_encoders, generation_config, model_type):
    ...
    def forward(
        ...
        if position_ids.max() > self.config.max_position_embeddings-1:
            ...
            print(position_ids)
            print(position_ids.max(),position_ids.min())
            assert 1>2
