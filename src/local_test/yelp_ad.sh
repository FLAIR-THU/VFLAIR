...
for seed in {60,61,62,63,64,65}
    do
    # 0.001
    python main_pipeline_llm.py --seed $seed --configs yelp_ad
    ...
    # 0.5
    sed -i 's/"lambda": 0.1/"lambda": 0.5/g' ./configs/yelp_ad.json
    # python main_pipeline_llm.py --seed $seed --configs yelp_ad

    # 1
    sed -i 's/"lambda": 0.5/"lambda": 1.0/g' ./configs/yelp_ad.json
    python main_pipeline_llm.py --seed $seed --configs yelp_ad
