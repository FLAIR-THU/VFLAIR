...
def load_configs(config_file_name, args):
    ...
    if 'defense_methods' in config_dict:
        ...
        defense_methods = []
        args.apply_trainable_layer = config_defense_dict['apply_trainable_layer'] if ('apply_trainable_layer' in config_attacker_dict) else 0
        args.apply_laplace = config_defense_dict['apply_laplace'] if ('apply_laplace' in config_attacker_dict) else 0
        args.apply_gaussian = config_defense_dict['apply_gaussian'] if ('apply_gaussian' in config_attacker_dict) else 0
        args.dp_strength = config_defense_dict['dp_strength'] if ('dp_strength' in config_attacker_dict) else 0.0
        args.apply_grad_spar = config_defense_dict['apply_grad_spar'] if ('apply_grad_spar' in config_attacker_dict) else 0
        args.grad_spars = config_defense_dict['grad_spars'] if ('grad_spars' in config_attacker_dict) else 0.0
        args.apply_encoder = config_defense_dict['apply_encoder'] if ('apply_encoder' in config_attacker_dict) else 0
        args.apply_adversarial_encoder = config_defense_dict['apply_adversarial_encoder'] if ('apply_adversarial_encoder' in config_attacker_dict) else 0
        args.ae_lambda = config_defense_dict['ae_lambda'] if ('ae_lambda' in config_attacker_dict) else 0.1
        args.encoder = config_defense_dict['encoder'] if ('encoder' in config_attacker_dict) else None
        args.apply_marvell = config_defense_dict['apply_marvell'] if ('apply_marvell' in config_attacker_dict) else 0
        args.marvell_s = config_defense_dict['marvell_s'] if ('marvell_s' in config_attacker_dict) else 0
    else:
