...
class NoisyLabel(Attacker):
    ...
    def attack(self):
        ...
        if self.args.apply_defense == True:
            exp_result = f"bs|num_class|Q|top_trainable|final_epoch|lr|acc,%d|%d|%d|%d|%d|%lf|%s|%lf|%lf|%s (AttackConfig: %s) (Defense: %s %s)" % (sample_count, self.num_classes, self.args.Q, self.args.apply_trainable_layer, self.vfl.epochs, self.vfl.lr, noise_type, noise_rate,self.vfl.test_acc, str(self.args.attack_configs), self.args.defense_name, str(self.args.defense_configs))
            
        else:
             exp_result = f"bs|num_class|Q|top_trainable|final_epoch|lr|noise_type|noise_rate|acc,%d|%d|%d|%d|%d|%lf|%s|%lf %lf" % (sample_count, self.num_classes, self.args.Q, self.args.apply_trainable_layer, self.vfl.epochs, self.vfl.lr, noise_type, noise_rate,self.vfl.test_acc)
        print(f'batch_size=%d,class_num=%d,noise_type=%s, noise_rate=%lf, main_acc=%lf' % (sample_count, self.num_classes, noise_type, noise_rate, self.vfl.test_acc))
