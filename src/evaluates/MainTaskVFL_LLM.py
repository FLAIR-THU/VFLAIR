...
class MainTaskVFL_LLM(object):
    ...
    def inference(self):
        ...
        test_targets = []
        with torch.no_grad():
            data_loader_list = [self.parties[ik].train_loader for ik in range(self.k)] # test_loader
            for parties_data in zip(*data_loader_list):
                print('parties_data[0]:',len(parties_data[0]), parties_data[0][0].shape)
                print('parties_data[1]:',len(parties_data[1]),parties_data[1][0].shape,parties_data[1][1].shape)
            ...
            print('Inference \t test_acc:{:.2f} test_auc:{:.2f}'.format(self.test_acc, self.test_auc))
            if self.noise_test_acc != None:
    ...
    def train(self):
        ...
        for i_epoch in range(self.epochs):
            ...
            self.current_step = 0
            for parties_data in zip(*data_loader_list):
                print('parties_data:',len(parties_data))
                print('parties_data[0]:',len(parties_data[0]))
                print(parties_data[0][0].shape)

                print('parties_data[1]:',len(parties_data[1]))

                print(parties_data[1][0].shape, parties_data[1][1].shape )
