        ...
        with torch.no_grad():
                ...
                if self.args.task_type == "SequenceClassification":
                    ...
                    if self.args.dataset == "Lambada":
                        ...
                        if self.args.metric_type == "best_pred":
                            ...
                            predict_word = [self.args.tokenizer.decode([_best_id]) for _best_id in
                                            predict_label.tolist()]
                            ...
                            for _bs in range(predict_label.shape[0]):  # each batch
                                predict_word = [self.args.tokenizer.decode([_label]) for _label in
                                                predict_label[_bs].tolist()]
                                predict_word = [normalize_answer(_p) for _p in predict_word]
            ...
            if self.args.task_type == "SequenceClassification":
                if self.num_classes == 1:
                    ...
                    exp_result = 'test_mse:{:.4f} test_pearson_corr:{:.4f} test_spearmanr_corr:{:.4f}'.format(
                        self.test_mse, self.test_pearson_corr, self.test_spearmanr_corr)
                    print(exp_result)
                    ...
                    self.test_mcc = matthews_corrcoef(np.array(test_predict_labels),
                                                      np.array(test_actual_labels))  # MCC
