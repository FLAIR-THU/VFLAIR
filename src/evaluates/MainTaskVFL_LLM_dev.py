...
def create_main_task(global_model_type):
                ...
                for parties_data in zip(*data_loader_list):
                    ...
                    for key_name in parties_data[0][0][0].keys():
                        ...
                        data_inputs[key_name] = torch.stack(
                            [parties_data[0][0][i][key_name] for i in range(len(parties_data[0][0]))])
 ...
 \
                        ...
                        batch_target_word, batch_predict_word, sample_cnt = self.generate_result(generation_output,
                                                                                                 gt_one_hot_label)
        ...
        def generate_result(self, model_output, gt_one_hot_label):
            ...
            if self.args.model_architect == 'CLS':  # task_type == "SequenceClassification":
                ...
                for i in range(start_logits.shape[0]):  # for each sample in this batch
                    ############ Gold ################
                    feature = parties_data[0][0][i][
                        'feature']  # print('parties_data[0][4]:',type(parties_data[0][4]),'feature:',type(feature))
