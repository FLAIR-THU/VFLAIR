...
class MainTaskVFLwithMID(object):
    ...
    def train_batch(self, parties_data, batch_label):
        ...
        # print(self.pred_gradients_list_clone)
        ...
        for ik in range(self.k):
            # if self.args.apply_mid == True and (ik in self.args.defense_configs['party']):
            #     if ik < self.k-1:
            #         # self.pred_gradients_list_clone[ik] = self.parties[ik].local_model.mid_gradients
            #         self.parties[ik].local_model_optimizer.zero_grad()
            #         self.parties[ik].local_model.mid_loss.backward(retain_graph=True)
            #         self.parties[ik].local_model_optimizer.step()
            #     else:
            #         # active party apply mids 
            #         pass
    ...
    def train(self):
        ...
        if self.args.apply_cae == True:
            exp_result = f"bs|num_class|epochs|recovery_rate,%d|%d|%d| %lf CAE wiht lambda %lf" % (self.batch_size, self.num_classes, self.epochs, self.test_acc, self.args.defense_configs['lambda'])
        elif self.args.apply_mid == True:
            exp_result = f"bs|num_class|epochs|recovery_rate,%d|%d|%d| %lf MID wiht party %s" % (self.batch_size, self.num_classes, self.epochs, self.test_acc, str(self.args.defense_configs['party']))
    ...
    def save_state(self):
        return {
            # "model": [self.parties[ik].local_model.detach() for ik in range(self.args.k)]+[self.parties[self.args.k-1].global_model],
