...
"textattackbert-base-uncased-CoLA": "D:\\work\\tools\\bert-base-uncased-CoLA",
...
"gpt2-large":"D:\\work\\tools\\gpt2-large",
...
# "rsvp-aibertserini-bert-base-squad": "/dev/data/sunl0/models/bertserini-bert-base-squad",
...
def load_basic_models_llm_bert(pretrained, task_type, model_type, current_model_type, current_output_dim, is_local, device):
    # current_model_type = args.model_list[str(index)]['type']
    # current_output_dim = args.model_list[str(index)]['output_dim']
...
def load_basic_models_llm(pretrained, task_type, model_type, current_model_type, current_output_dim, is_local, device):
    if model_type in ['Bert','Albert','Roberta']:
        local_model, local_model_optimizer, global_model, global_model_optimizer, tokenizer = load_basic_models_llm_bert(
            pretrained, task_type, model_type, current_model_type, current_output_dim, is_local, device
        )
...
def load_models_per_party(pretrained, task_type, model_type, current_model_type, current_output_dim, is_local, device):
    if current_model_type in LLM_supported:
        local_model, local_model_optimizer, global_model, global_model_optimizer, tokenizer = load_basic_models_llm(
            pretrained, task_type, model_type, current_model_type, current_output_dim, is_local, device
        )
